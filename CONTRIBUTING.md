...
- Clone the forked repository to your local development machine
  (e.g. `git clone git@github.com:<YOUR_GITHUB_USER>/audio_waveforms.git`).
...
- When committing your changes, make sure that each commit message is clear
  (e.g. `git commit -m 'Add waveform extraction controller.'`).
...
Example (if the current version is v2.0.1):

```dart
@Deprecated('Will be removed in v3.0.0, use nonDeprecatedFeature() instead')
...
mark (e.g. `feat!: Add noOfSamplesPerSecond parameter to preparePlayer for easier waveform configuration`).

Examples of PR titles:

- feat: ✨ Add `noOfSamplesPerSecond` parameter to `preparePlayer`
- fix: 🐛 Fixed labels not showing properly while recording
- docs: 📚 Update analysis options and related files
- chore: 🔧 Update compileSdk and targetSdk to run example app
- test: 🚨 Add unit test for waveform extraction
- refactor: 🔨 Optimize waveform extraction performance
